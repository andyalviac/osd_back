...
from src.Infrastructure.Grpc.Generated import Location_pb2_grpc
...
class LocationClient:
    ...
    def __init__(self, host=Config.SECURITY_GRPC_HOST, port=Config.SECURITY_GRPC_PORT):
        if not host:
            raise OSDException("LOCATION_GRPC_HOST is not set")
        self.host = host
        self.port = port
    ...
    @grpc_retry
    async def get_location_by_id(self, request) -> Response:
        req = Common_pb2.Request(data=request)
        return await self.stub.GetLocationDataById(req)
